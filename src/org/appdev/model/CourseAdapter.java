...
public class CourseAdapter extends DataBaseAdapter {
    ...
    public List<CourseAdapter> getCourseList(Boolean isSimple)
    {
        ...
        Cursor cursor=dbInstance.query(CourseAdapter.TABLE_NAME, null, null, null, null, null, ID+" DESC");
        if (cursor.moveToFirst() == false) {
        	return null;
        }
        while(!cursor.isAfterLast()&& (cursor.getString(1)!=null)){
            CourseAdapter course=new CourseAdapter(this.context);
            course.setId(cursor.getInt(0));
            course.setCourseName(cursor.getString(cursor.getColumnIndex(CourseAdapter.NAME)));   
            course.setSchoolName(cursor.getString(cursor.getColumnIndex(CourseAdapter.SCHOOL)));
            course.setVisited(cursor.getLong(cursor.getColumnIndex(CourseAdapter.VISITED)));
            course.setCourseGUID(cursor.getString(cursor.getColumnIndex(CourseAdapter.COURSE_GUID)));
            course.setCourseVersion(cursor.getString(cursor.getColumnIndex(CourseAdapter.COURSE_VERSION)));
    ...
    public long create(String name, String version, String school, String guid, long visited, Bitmap courseIcon) {
    	ContentValues values = new ContentValues();

        values.put(NAME, name);
        values.put(SCHOOL, school);
        values.put(DIR_NAME, school);
    ...
    public Long saveCourse(CourseAdapter course, Bitmap courseIcon)
    {
        ...
        values.put(SCHOOL, course.getSchoolName());
    ...
    public Cursor fetch(long rowId) throws SQLException {
        Cursor mCursor =
            dbInstance.query(true, TABLE_NAME, new String[] {ID,
                   NAME, VISITED, COURSE_ICON, COURSE_GUID, SCHOOL, COURSE_VERSION}, ID + "=" + rowId, null,
                    null, null, null, null);
        if (mCursor != null) {
            mCursor.moveToFirst();
        }
    ...
    public Cursor fetchByGUID(String guid) throws SQLException {
        Cursor mCursor =
            dbInstance.query(true, TABLE_NAME, new String[] {ID,
                   NAME, VISITED, COURSE_ICON, COURSE_GUID, SCHOOL, COURSE_VERSION}, COURSE_GUID + "=\"" + guid + "\"", null,
                    null, null, null, null);
        if (mCursor != null) {
            mCursor.moveToFirst();
        }
    ...
    public Cursor fetchAll() {
    	return dbInstance.query(TABLE_NAME, new String[] {ID,
                NAME, VISITED, COURSE_ICON, COURSE_GUID, SCHOOL, COURSE_VERSION}, null, null, null, null, null);
    }
