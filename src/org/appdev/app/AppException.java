...
public class AppException extends Exception implements UncaughtExceptionHandler{
	...
	private final static boolean Debug = true;//True: set to save error log
	...
	public void saveErrorLog(Exception excp) {
		...
		try {
			...
			if(storageState.equals(Environment.MEDIA_MOUNTED)){
				savePath = FileUtils.EkkoFilePath() + "Log/";
			...
			//SD card not mounted or no write permission
			if(logFilePath == ""){
				return;
			}
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			public void run() {
				Looper.prepare();
				UIController.sendAppCrashReport(context, crashReport);
				Looper.loop();
			}
